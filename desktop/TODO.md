...
- [ ] **[Alta] Capítulos recebidos via `sync-comic` não persistem todos, com `UNIQUE constraint violated`** - Reportado em 22/08/2026: sync de "Omoide Emanon" do Android pro Desktop, 10 headers de capítulo recebidos pelo fio (Ch.1 a Ch.9.5), mas só 8 apareceram na biblioteca depois (`comic:chapters` emitiu 8 itens). Log mostra 5x `UNIQUE constraint violated` bem entre o header de Ch.5 e o de Ch.6 — timing bate com os primeiros 5 capítulos recebidos (1-5), sugerindo que `persist_received_chapter` tentou inserir uma linha que já existia (possivelmente de uma tentativa de sync anterior/interrompida, já que também aparece `.incoming-3168404980656979692.tmp` sendo "skipped by scanner" repetidamente — sobra de uma transferência anterior que não foi limpa). Usuário também relatou ter precisado mandar reenviar, e da segunda vez só 1 item foi de fato transferido. Não investigado a fundo ainda — candidatos: (1) `persist_received_chapter` não trata `UniqueViolation` de forma alguma (deveria fazer upsert ou skip gracioso em vez de deixar a query falhar), (2) pode haver uma corrida entre duas sessões de sync sobrepostas (guard não pegando o caso), (3) arquivo `.tmp` órfão de uma tentativa anterior interrompida (relacionado ao item do `FsStore` acima — o app pode ter travado no meio de uma transferência antes da mitigação `.mem()` ser aplicada) pode estar confundindo o scanner/persistência local.
