...
impl ChapterService {
    ...
    pub async fn get_comic_chapters(
        ...
        let chapter_metas = if let Ok(Some(comic_meta)) = self.metadata_repo.get_comic_metadata_by_comic_id(comic_directory_fk).await {
            self.metadata_repo.get_chapter_metadata_by_comic_metadata_id(comic_meta.id).await.unwrap_or_default()
        } else {
            Vec::new()
        };

        let get_meta = |ch: &String| {
            chapter_metas.iter().find(|m| &m.chapter == ch)
        };
