pub async fn setup_test_db() -> sqlx::SqlitePool {
    ...
    // archive
    sqlx::query(include_str!("../../../migrations/models/archive/001_create_chapter_template.sql"))
        .execute(&pool)
        .await
        .unwrap();

    sqlx::query(include_str!("../../../migrations/models/archive/002_create_comic_directory.sql"))
        .execute(&pool)
        .await
        .unwrap();

    sqlx::query(include_str!("../../../migrations/models/archive/003_create_chapter_archive.sql"))
        .execute(&pool)
        .await
        .unwrap();
    ...
    sqlx::query(include_str!("../../../migrations/models/metadata/003_create_chapter_page.sql"))
        .execute(&pool)
        .await
        .unwrap();
    ...
    // category
    sqlx::query(include_str!("../../../migrations/models/category/001_create_category.sql"))
        .execute(&pool)
        .await
        .unwrap();

    sqlx::query(include_str!("../../../migrations/models/category/002_create_manga_category.sql"))
        .execute(&pool)
        .await
        .unwrap();

    // history
    sqlx::query(include_str!("../../../migrations/models/history/001_create_reading_history.sql"))
        .execute(&pool)
        .await
        .unwrap();

    sqlx::query(include_str!("../../../migrations/models/history/002_create_chapter_read.sql"))
        .execute(&pool)
        .await
        .unwrap();

    // views
    sqlx::query(include_str!("../../../migrations/views/001_create_comic_summary_view.sql"))
        .execute(&pool)
        .await
        .unwrap();
