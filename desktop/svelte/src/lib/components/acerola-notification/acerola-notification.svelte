	...
	{#snippet trigger()}
		<AcerolaButtonIcon class="relative">
			...
			{#if notifications.length > 0}
				<span
					class="absolute top-1.5 right-1.5 size-2 rounded-full border-2 border-background bg-primary"
				></span>
			{/if}
		</AcerolaButtonIcon>
	{/snippet}
		...
		>
			...
			{:else}
				{#each notifications as notify (notify.id)}
					...
					>
						<Icon size={18} class={notifyVariants({ variant: notify.variant })} />
						...
						<AcerolaButtonIcon class="size-6 shrink-0" onclick={() => pop(notify.id)}>
							<XIcon size={12} />
						</AcerolaButtonIcon>
					</div>
				{/each}
