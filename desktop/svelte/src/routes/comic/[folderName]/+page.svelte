<!-- VERSION: 7.0 - STABLE VIRTUALIZATION -->
<script lang="ts">
  ...
  $effect(() => {
    ...
    untrack(() => {
      ...
      chapterStore.clear();
      if (comic) {
        chapterStore.fetch(comic.relations.directoryId, 0, parseInt(chaptersPerPage), isAscending, vol);
      }
    });
  });
  ...
  $effect(() => {
    ...
    const missingPages = visiblePages.filter(p => !chapterStore.lruKeys.includes(p));
    if (missingPages.length > 0) {
      untrack(() => {
        chapterStore.fetch(comic.relations.directoryId, missingPages[0], parseInt(chaptersPerPage), isAscending, expandedVolumeId);
      });
    }
  });
  ...
  const manga = $derived.by(() => {
    ...
    const pagesData = (chaptersData?.pages ?? []).map(p => ({
      page: p.page,
      items: p.items.filter(c => {
        const matchesSearch = c.name.toLowerCase().includes(searchQuery.toLowerCase());
        const matchesVolume = !expandedVolumeId || c.volumeId === expandedVolumeId;
    ...
    const volumes = (chaptersData?.archive.volumes ?? []).map(v => ({
      ...
      hasMore: expandedVolumeId === v.id.toString() && pagesData.flatMap(p => p.items).length < v.chapterCount,
      chapters: [] // não passamos mais flat chapters, os childs usam pagesData
    }));
    ...
    return {
      ...
      volumes,
      pageSize,
      metadata: { description: "Descrição indisponível.", author: "Autor Desconhecido", status: "Desconhecido", source: item.metadata.activeSource || "LOCAL", genres: [] }
    };
  });
</script>

{#if manga}
  <div in:fade={{ duration: 200 }} out:fade={{ duration: 200 }} class="fixed inset-0 z-50 bg-base overflow-hidden flex h-screen text-text">
    <div class="absolute inset-0 overflow-hidden pointer-events-none">
      <img src={manga.banner || manga.cover} alt="" class="w-full h-full object-cover scale-150 blur-[120px] opacity-20" referrerpolicy="no-referrer" />
      <div class="absolute inset-0 bg-base/40"></div>
    </div>
    <ComicMetadataPanel title={manga.title} author={manga.metadata.author} status={manga.metadata.status} source={manga.metadata.source} chaptersCount={manga.chaptersCount} description={manga.metadata.description} cover={manga.cover} {onBack} />
    <div class="flex-1 overflow-y-auto scrollbar-hide relative z-10 flex flex-col [overflow-anchor:none]">
      <div class="lg:hidden sticky top-0 z-50 flex items-center justify-between px-6 h-20 bg-base/90 backdrop-blur-md border-b border-surface/30">
        <AcerolaButtonIcon onclick={onBack} size="sm"><ArrowLeft size={20} /></AcerolaButtonIcon>
      ...
      <div class="max-w-5xl w-full mx-auto p-8 lg:p-16 space-y-12">
        <div class="sticky top-0 z-40 bg-base/5 backdrop-blur-3xl border-b border-surface/30 px-4 -mx-4 flex items-center justify-between">
          <AcerolaToggleGroup type="single" bind:value={activeTab} class="flex gap-4">
            {#snippet children()}
              <ToggleGroupItem value="content" class="py-6 bg-transparent border-none text-sm font-black uppercase tracking-[0.2em] relative data-[state=on]:text-primary data-[state=on]:bg-transparent">
                {chapterStore.chapters?.hasVolumeStructure ? "Volumes" : "Capítulos"}
                {#if activeTab === "content"}<div class="absolute bottom-0 left-0 right-0 h-1 bg-primary rounded-full" in:fade></div>{/if}
              </ToggleGroupItem>
              <ToggleGroupItem value="preferences" class="py-6 bg-transparent border-none text-sm font-black uppercase tracking-[0.2em] relative data-[state=on]:text-primary data-[state=on]:bg-transparent">
                Preferências
                {#if activeTab === "preferences"}<div class="absolute bottom-0 left-0 right-0 h-1 bg-primary rounded-full" in:fade></div>{/if}
              </ToggleGroupItem>
            {/snippet}
          </AcerolaToggleGroup>
          {#if activeTab === "content" && !chapterStore.chapters?.hasVolumeStructure}
            <div class="flex items-center gap-2 pr-4">
              <input type="text" bind:value={searchQuery} placeholder="FILTRAR..." class="bg-surface/20 border border-surface/30 rounded-full py-2 px-6 text-[10px] font-black tracking-widest focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all w-40 focus:w-60" />
            </div>
          {/if}
        </div>
        <div class="min-h-150">
          {#if activeTab === "content"}
            {#if chapterStore.chapters?.hasVolumeStructure}
              <ComicVolumeList volumes={manga.volumes} pagesData={manga.pagesData} loading={chapterStore.loading} pageSize={manga.pageSize} onexpand={v => expandedVolumeId = v} onvisiblepages={p => visiblePages = p} />
            {:else}
              <ComicChapterList pagesData={manga.pagesData} totalChapters={manga.chaptersCount} pageSize={manga.pageSize} onvisiblepages={p => visiblePages = p} />
            {/if}
