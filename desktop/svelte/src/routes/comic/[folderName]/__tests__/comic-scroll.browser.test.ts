...
describe("ComicPage Scroll Integration", () => {
  ...
  const loaderDataMock = {
    comic: {
      ...
      metadata: { title: "Production Test Manga", externalSync: false, activeSource: "LOCAL", chapterCount: TOTAL_CHAPTERS },
  ...
  it("should fetch the correct next page when scrolling down (Bidirectional Flow)", async () => {
    ...
    await vi.waitFor(() => {
        ...
        const page1Requested = invokeCalls.some((callArgs: any[]) => 
  ...
  it("should render correct chapter titles using the clean name from Rust", async () => {
    ...
    await vi.waitFor(() => {
        const chapterTitles = Array.from(container.querySelectorAll('[data-slot="item-title"]'))
