...
| --- | --- |
| Desktop (Rust + Tauri + Svelte) | [platforms/desktop/CONTRIBUTING.md](platforms/desktop/CONTRIBUTING.md) |
...
- **Escopo por PR**: um PR deve tocar uma única plataforma (`platforms/android/`, `platforms/desktop/`, `platforms/relay/`, `lib/p2p/` ou `lib/relay/`), salvo mudanças de fato compartilhadas (docs raiz, `LICENSE`, `PRIVACY_POLICY.md`).
- **Convenção de commit**: `[tag](plataforma): descrição`, por exemplo `[fix](desktop): corrige leak de conexão no reader`. O `tag` segue os mesmos prefixos documentados nos guias específicos (`feat`, `fix`, `docs`, `refactor`, `test`, `chore`, `ci`, `merge`); a `plataforma` é `android`, `desktop`, `p2p`, `relay` (para `platforms/relay/`), `relay-lib` (para `lib/relay/`, o código vendorizado do iroh) ou `monorepo` para mudanças na raiz.
