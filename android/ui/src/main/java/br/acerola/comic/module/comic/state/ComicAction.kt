...
sealed interface ComicAction {
    ...
    data class SelectTab(val tab: MainTab) : ComicAction
    data class UpdatePageSize(val size: ChapterPageSizeType) : ComicAction
    data class UpdateCategory(val categoryId: Long?) : ComicAction
    data class ToggleExternalSync(val enabled: Boolean) : ComicAction
}

sealed interface ComicChapterAction {
    data class ChangePage(val page: Int) : ComicChapterAction
    data class ClickChapter(val chapter: ChapterFileDto, val initialPage: Int) : ComicChapterAction
    data class ClickContinue(val chapterId: Long, val lastPage: Int) : ComicChapterAction
    data class ToggleReadStatus(val chapterId: Long) : ComicChapterAction
}
