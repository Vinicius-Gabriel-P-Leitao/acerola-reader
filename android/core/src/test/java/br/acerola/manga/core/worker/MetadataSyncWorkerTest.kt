...
class MetadataSyncWorkerTest {
    ...
    @MockK
    lateinit var syncMangaMetadataUseCase: SyncMangaMetadataUseCase
    ...
    // Test for directoryId != -1L (Single manga sync)
    @Test
    fun `doWork should return success when syncFromMangadex for single manga succeeds`() = runBlocking {
        coEvery { syncMangaMetadataUseCase.syncFromMangadex(42L) } returns Either.Right(Unit)
    ...
    @Test
    fun `doWork should return failure when syncFromMangadex for single manga fails`() = runBlocking {
        coEvery { syncMangaMetadataUseCase.syncFromMangadex(42L) } returns Either.Left(
    ...
    @Test
    fun `doWork should return success when syncFromAnilist for single manga succeeds`() = runBlocking {
        coEvery { syncMangaMetadataUseCase.syncFromAnilist(42L) } returns Either.Right(Unit)
    ...
    @Test
    fun `doWork should return success when syncFromComicInfo for single manga succeeds`() = runBlocking {
        coEvery { syncMangaMetadataUseCase.syncFromComicInfo(42L) } returns Either.Right(Unit)
    ...
    @Test
    fun `doWork should return success when mangadex library sync succeeds`() = runBlocking {
    ...
    @Test
    fun `doWork should return failure when mangadex library sync fails`() = runBlocking {
        coEvery { mangadexSyncUseCase.sync(null) } returns Either.Left(
            LibrarySyncError.SyncNetworkError()
        )
    ...
    @Test
    fun `doWork should return success when anilist library rescan succeeds`() = runBlocking {
    ...
    @Test
    fun `doWork should return success when comicInfo library sync succeeds`() = runBlocking {
    ...
    @Test
    fun `doWork should handle exceptions gracefully and return failure`() = runBlocking {
