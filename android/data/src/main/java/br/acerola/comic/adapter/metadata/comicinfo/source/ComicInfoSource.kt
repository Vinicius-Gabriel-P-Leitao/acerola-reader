    ...
    constructor(
        ...
        override suspend fun searchInfo(
            ...
            withContext(context = Dispatchers.IO) {
                val folderUri =
                    extra.getOrNull(index = 0)?.toUri()
                        ?: return@withContext Either.Left(
                            value = NetworkError.UnexpectedError(cause = Exception("Folder URI missing in extra[0]")),
                        )
                ...
                val firstChapter =
                    folderDoc.listFiles().firstOrNull {
