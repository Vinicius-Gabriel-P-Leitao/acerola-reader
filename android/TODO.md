...
- [ ] **`browse-library` v2: sinal de versão de capa na resposta** *(Média)* - Resposta do `acerola/browse-library/1` (`FfiComicSummaryEntry`) ganha um campo `cover_version` (reaproveitando `ComicDirectory.lastModified` já existente no Room, sem hash novo) pra o peer decidir se precisa buscar capa nova via o protocolo `browse-cover` abaixo. `getLibrarySummary()` já foi corrigido pra não tocar SAF/`DocumentFile` (`ChapterArchiveDao.getLibrarySummary`, só SQLite) — esse item só adiciona o campo de versão em cima disso.
- [ ] **Novo protocolo `acerola/browse-cover/1`** *(Baixa)* - Protocolo dedicado pra buscar a capa (thumbnail) de um quadrinho remoto por vez, complementar ao `browse-library`: outbound manda `{ comic_name, known_version }`, inbound responde `not_modified` se a versão bater ou os bytes da capa (reaproveitando os handles de leitura de arquivo já usados por `sync-files`, `protocol/files/exchange.rs`) se não. Cliente busca em paralelo — várias streams simultâneas na mesma conexão já pooled por `(peer, alpn)` — só das capas visíveis na lista, cacheando localmente por `(peer_id, comic_name, cover_version)` pra nunca rebaixar a mesma capa duas vezes.
- [ ] **Migrar `sync-files`/`sync-comic` pra usar blobs do `acerola-p2p`** *(Baixa)* - Depende da abstração de blobs na lib (ver `TODO.md` do `acerola-p2p`). Troca o protocolo manual de manifesto/want-list/chunking atual (`protocol/files/exchange.rs`) por transferência content-addressed nativa (dedup, resume e verificação de integridade automáticos), sem precisar manter a maquinaria de chunking/checksum feita à mão.
